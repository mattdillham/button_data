function init() {
  ...
  function buildBarChart(sample) {
    d3.json("samples.json").then((data) => {
      ...
      var result = resultArray[0];
      var otu_ids = result.otu_ids.slice(0, 10).map(numericIds => {
    ...
    function buildGaugeChart(sample) {
      ...
      function buildBubbleChart(sample) {
        d3.json("samples.json").then((data) => {
          ...
          var sample_value = result.sample_values.reverse();
          var otu_label = result.otu_labels.reverse();
          var bubble_trace = {
            x: otu_ids,  
            y: sample_value,
            text: otu_label,
